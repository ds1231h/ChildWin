...
LRESULT CALLBACK MainWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_SIZE:
		...
		for (y=0; y<ROW_NUM; y++)
		{
			MoveWindow (hChildWnd[x][y], x*(LOWORD(lParam) / 8) / COL_NUM, y*HIWORD(lParam) / ROW_NUM, nXBox, nYBox, TRUE);
		}
		return 0;
    ...
    case WM_COMMAND:                //子窗口被命中的通知消息
		...
		newId = nChildWin_ID + 1;
		InvalidateRect(hWnd, NULL, TRUE);// 生成新的nChildWin_ID并传值赋值后需重绘窗口
		return 0;
	...
	case WM_PAINT:
		...
		wsprintf(posi, TEXT("窗口序号：%d"), newId);
		DrawText(hdc, posi, -1, &rect, DT_SINGLELINE | DT_CENTER | DT_VCENTER);
		EndPaint(hWnd, &ps);
		return 0;
...
LRESULT CALLBACK ChildWndProc (HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
		...
		if (GetWindowLong (hWnd, 0))
		{
			...
			// 使用Rectangle函数
			hBrush= CreateSolidBrush(RGB((BYTE)(newId*10), 255 - (BYTE)(newId*10), 120 + (BYTE)(newId*5)));
